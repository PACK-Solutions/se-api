...
class ConnaissanceClientHistoriqueProjectionTest : ShouldSpec(
    {
        ...
        should("record modifications on proche PPE") {
            ConnaissanceClientHistoriqueProjection(
                Old(aConnaissanceClient().defaultValue()),
                New(aConnaissanceClient().withStatutProchePPE(LienParente.CONJOINT, FonctionPPE.DIRIGEANT_PARTI).build()),
            ).getChangements() shouldBe setOf(
        ...
        should("record modifications on motifs vigilance") {
            ConnaissanceClientHistoriqueProjection(
                ...
                New(aConnaissanceClient().withVigilanceRenforcee(MotifVigilance.MONTANT_ELEVE, MotifVigilance.AGE_AVANCE).build()),
            ).getChangements() shouldContain
